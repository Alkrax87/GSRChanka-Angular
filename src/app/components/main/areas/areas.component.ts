...
export class AreasComponent {
  ...
  constructor(private _matDialog: MatDialog) {
    const acoleccion = collection(this.firebase, 'areas');
  ...
  combineLatest([
    ...
    map(([projects, searchText]) => {
      ...
      const filtered = projects.filter(proyecto => 
        proyecto.nombreArea.toLowerCase().includes(searchValue) || 
        proyecto.idUsuario.toLowerCase().includes(searchValue) || 
        proyecto.nroEmpleados.toLowerCase().includes(searchValue) || 
        proyecto.Observaciones.toLowerCase().includes(searchValue) 
      );
      return filtered;
    })
...
//Metodo para seleccionar cantidad de proyectos en una pagina
 ...
 // Método para generar el texto dinámico en la tabla
  ...
  abrirModalCreate(): void {
    this._matDialog.open(CreateAreaComponent, {
      width: '700px'
    });
  }
  //Modal para abrir el Editar-Proyecto
  abrirModalEditar(proyecto: any): void {
    this._matDialog.open(EditAreaComponent, {
      width: '700px',
  ...
  abrirModalMostrar(proyecto: any): void {
    this._matDialog.open(ShowAreaComponent, {
      width: '700px',
  ...
  eliminarProyecto(id: string): void {
    const dialogRef = this._matDialog.open(DeleteAreaComponent, {
      width: '400px',
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        const areasDocRef = doc(this.firebase, `areas/${id}`);
        deleteDoc(areasDocRef)
          .then(() => {
