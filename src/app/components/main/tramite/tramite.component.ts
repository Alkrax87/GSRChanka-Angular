...
import { DeleteTramiteComponent } from './delete-tramite/delete-tramite.component';
...
export class TramiteComponent {
  ...
  constructor(private _matDialog: MatDialog) {
    const acoleccion = collection(this.firebase, 'tramites');
  ...
  combineLatest([
    ...
    map(([projects, searchText]) => {
      ...
      const filtered = projects.filter(proyecto => 
        proyecto.nroDocumento.toLowerCase().includes(searchValue) || 
        proyecto.tipoDocumento.toLowerCase().includes(searchValue) || 
        proyecto.idUsuario.toLowerCase().includes(searchValue) || 
        proyecto.areaOrigen.toLowerCase().includes(searchValue) || 
        proyecto.areaDestino.toLowerCase().includes(searchValue) || 
        proyecto.estadoDocumento.toLowerCase().includes(searchValue) || 
        proyecto.nroFolios.toLowerCase().includes(searchValue) || 
        proyecto.asuntoTramite.toLowerCase().includes(searchValue) || 
        proyecto.fechaTramite.toLowerCase().includes(searchValue) || 
        proyecto.archivoAdjunto.toLowerCase().includes(searchValue)
      );
      return filtered;
    })
...
//Metodo para seleccionar cantidad de proyectos en una pagina
 ...
 // Método para generar el texto dinámico en la tabla
  ...
  abrirModalCreate(): void {
    this._matDialog.open(CreateTramiteComponent, {
      width: '700px'
    });
  }
  //Modal para abrir el Editar-Proyecto
  abrirModalEditar(tramite: any): void {
    this._matDialog.open(EditTramiteComponent, {
      width: '700px',
      data: tramite // Pasar los datos del proyecto al modal de edición
    });
  }
  //Metodo para Show
  abrirModalMostrar(tramite: any): void {
    this._matDialog.open(ShowTramiteComponent, {
      width: '700px',
      data: tramite // Pasar los datos del proyecto al modal
    });
  }
  //Metodo eliminar un proyecto
  eliminarProyecto(id: string): void {
    const dialogRef = this._matDialog.open(DeleteTramiteComponent, {
      width: '400px',
    });
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        const proyectoDocRef = doc(this.firebase, `tramites/${id}`);
